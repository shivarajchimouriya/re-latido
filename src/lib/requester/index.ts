...
export class APIService implements IFetchRequest {
  ...
  private async fetchWrapper<T>({
    ...
    data,
    method,
  }: IApiRequestOptions): Promise<T> {
    //alll the CRUD requests go via this function and request interceptions are handeled here
    const apiUrl = this.prepareUrl({
  ...
  async get<T>({ options, url }: RequestInterfaceClient): Promise<T> {
    ...
    const res = await this.fetchWrapper<T>({
      url,
      data: undefined,
